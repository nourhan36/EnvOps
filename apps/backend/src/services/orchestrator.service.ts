...
function buildEmulatorKubeConfig(): k8s.KubeConfig {
    ...
    try {
        ...
        // No default kubeconfig available.
        // Fall through to the AWS CLI fallback.
    }
    ...
    if (execUser?.exec) {
        ...
        kc.addUser({
            name: userName,
            authProvider: execUser.authProvider,
        });
    } else {
        kc.addUser({
            name: userName,
 ...
 *
 * IMPORTANT:
 * The backend runs INSIDE EKS, so for the real AWS/EKS environment
 * we must use loadFromCluster().
 *
 * loadFromDefault() is for a local kubeconfig such as:
 * ~/.kube/config
 *
 * loadFromCluster() reads:
...
if (env.kubernetesTarget === 'emulator') {
    ...
    try {
        ...
        console.log('Kubernetes client configured using in-cluster authentication.');
    } catch (error) {
        console.error(
            'Failed to load in-cluster Kubernetes configuration:',
            error
        );
...
export interface ProvisionRequest {
    ...
    /** Optional pod args passed to the image entrypoint. */
    args?: string[];
...
/**
 * Build security context for sandbox pods.
 */
export function buildSecurityContext(
    mode: SecurityMode = 'hardened'
) {
    if (mode === 'privileged') {
        // Trusted runtime templates only.
        // Docker-in-Docker and k3s may require privileged access.
        return {
            runAsUser: 0,
            runAsNonRoot: false,
            privileged: true,
    ...
    if (mode === 'root') {
        // Runs as root but is not privileged.
        return {
            runAsUser: 0,
            runAsNonRoot: false,
            privileged: false,
...
/**
 * Create a sandbox namespace, pod and service.
 */
export async function provisionSandbox(
    request: ProvisionRequest
): Promise<ProvisionResult> {
    ...
    console.log(
        `Provisioning sandbox -> Image: ${request.dockerImage}, ` +
        `Limits: ${JSON.stringify(request.limits)}, ` +
        `Security: ${mode}`
    );
    ...
    try {
        ...
        console.log(
            `Created namespace: ${namespaceName}`
        );
        ...
        console.log(
            `Created pod ${podName} in namespace ${namespaceName}`
        );

        /**
         * 3. Create sandbox service
         */
        await coreV1Api.createNamespacedService({
            namespace: namespaceName,
        ...
        console.log(
            `Created service in namespace ${namespaceName}`
        );

        /**
         * 4. Wait until pod is running
         */
        ...
        if (!isReady) {
            throw new Error(
                'Pod failed to reach Running state within timeout.'
            );
        }

        console.log(
            `Sandbox ${namespaceName} is running.`
        );
        ...
        console.error(
            `Failed to provision sandbox in ${namespaceName}:`,
            error
        );

        /**
         * Cleanup namespace after failed provisioning.
         */
        await coreV1Api
            .deleteNamespace({
                name: namespaceName,
...
/**
 * Poll sandbox pod until it reaches Running.
 */
async function waitForPodRunning(
    namespace: string,
    podName: string,
    ...
    const maxAttempts = Math.ceil(
        env.kubernetesProvisionTimeoutMs /
            pollIntervalMs
    );
    ...
    while (attempts < maxAttempts) {
        ...
        let podStatus:
            k8s.V1PodStatus | undefined;

        try {

            const podResponse =
                await coreV1Api.readNamespacedPodStatus({
                    name: podName,
                    namespace,
                });

            podStatus =
                (podResponse as any).body?.status ??
                (podResponse as any).status;
            ...
            if (
                ...
                throw new Error(
                    `Pod ${podName} disappeared before it became ready.`
                );
            }

            console.warn(
                `Transient error reading pod status in ${namespace}:`,
                error?.message ?? error
            );
        ...
        /**
         * Pod has a terminal failure.
         */
        if (diagnostic.failure) {

            throw new Error(
                `Provisioning failed for image ${image}: ` +
    ...
    throw new Error(
        `Pod failed to reach Running state within ` +
        `${env.kubernetesProvisionTimeoutMs}ms. ` +
        (
            lastFailure
                ? `Last status: ${lastFailure}.`
...
/**
 * Container states that are considered terminal.
 */
const TERMINAL_WAITING_REASONS =
    new Set([
        'ErrImagePull',
        'InvalidImageName',
        'CreateContainerConfigError',
...
/**
 * Detect Docker Hub/image registry rate-limit errors.
 */
...
/**
 * Diagnose Kubernetes pod status.
 */
export function diagnosePodStatus(
    status: k8s.V1PodStatus | undefined
): PodDiagnostic {
    ...
    /**
     * Pod itself failed or succeeded.
     */
    if (
        phase === 'Failed' ||
        phase === 'Succeeded'
        ...
        return {
            ...
            failure: {
                ...
                message:
                    ...
                    'the pod could not be scheduled to any node',
            },
        };
    }
    ...
    for (
         ...
         * ImagePullBackOff.
         *
         * Allow temporary Docker Hub rate limits.
         */
        if (
            waiting?.reason ===
                'ImagePullBackOff' &&
            ...
            return {
                ...
                failure: {
                    ...
                    message:
                        waiting.message ??
                        `image pull for ${container.name} is failing`,
                },
            };
        }
        ...
        if (
            ...
            return {
                ...
                failure: {
                    ...
                    message:
                        `container ${container.name} ` +
                        `restarted ${container.restartCount} time(s): ` +
                        (
                            terminated.message ??
                            terminated.reason ??
                            'unknown reason'
...
export async function cleanupSandbox(
    ...
    console.log(
        `Executing garbage collection for namespace: ${namespace}`
    );
    ...
    try {
        ...
        console.log(
            `Garbage collection triggered successfully for: ${namespace}`
        );
        ...
        console.error(
            `Critical: Failed to delete namespace ${namespace}:`,
            error
        );

        throw new Error(
            `Garbage collection failed for ${namespace}`
        );
    }
}

/**
 * Attach terminal to sandbox pod.
 */
export async function attachTerminal(
    request: AttachTerminalRequest
): Promise<any> {
    ...
    console.log(
        `Attaching terminal to namespace: ${request.namespace}`
    );
    ...
    try {
        ...
        console.error(
            `Critical: Failed to attach terminal ` +
            `for ${request.namespace}:`,
            error
        );

        throw new Error(
            'Terminal connection to the sandbox failed.'
        );
    }
}

/**
 * Delete all resources belonging to sandbox namespace.
 */
export async function deleteSandboxResources(
    ...
    console.log(
        `Executing sandbox resource deletion for namespace: ${namespace}`
    );
    ...
    try {
        ...
        console.log(
            `Successfully deleted sandbox resources ` +
            `(namespace & pods) for: ${namespace}`
        );
        ...
        console.error(
            `Critical: Failed to delete sandbox resources ` +
            `for ${namespace}:`,
            error
        );

        throw new Error(
            `Failed to delete sandbox resources for ${namespace}`
        );
    }
}